...
async function Withdraw() {
  ...
  } else console.log("Please enter a valid amount to withdraw.");
};
...
async function QuickDeposit() {
  ...
  if (qckDpzt.QuickDeposit <= 50000) {
    console.log(
      `The amount has been transferred to your account. Your new Balance is`,
      qckDpzt.QuickDeposit + Balance
    );
    console.log("\n\n\t\tThanks for using the services of Nexa-Vault ATM");
  } else if (qckDpzt.QuickDeposit >= 50000) {
    console.log(
      " We apologize, but the deposit limit for this transaction has been exceeded.\n Please Visit the nearest branch for further assistance."
    );
  } else {
...
if (pinInput.Input === Pin) {
  ...
  setTimeout(() => {
    console.log(
      " Enter a 4-Digit Pincode & Enjoy Exploring the App \n Your Feedbacks would be appriciated :) "
    );
